...
def run_mb(thickness: float, instant_recomb: int, transient: bool, gap: bool):
    ...
    folder = "standard_case".format(thickness)
    if transient:
        folder += "/transient"
    else:
        folder += "/steady_state"
    ...
    if transient:
        ...
        my_model.settings.final_time = 1e7#31536000
    else:
        my_model.dt = None
        my_model.settings.transient = False
        my_model.settings.final_time = None

    #my_model.settings.absolute_tolerance=1e4,
    #my_model.settings.relative_tolerance=1e-5,
